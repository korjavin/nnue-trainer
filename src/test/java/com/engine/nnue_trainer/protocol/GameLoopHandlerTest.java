...
class GameLoopHandlerTest {
  ...
  void setUp() {
    ...
    handler =
        new GameLoopHandler(objectMapper) {
          @Override
  ...
  void testTurnChangeTriggersMakeMove() {
    ...
    handler.handleMessage(
        "{\"type\":\"multiplayer_game_start\",\"playerIndex\":0,\"rows\":6,\"cols\":7}");
    ...
    assertFalse(
        makeMoveCalled, "makeMove should NOT be called when turn changes to another player.");
  }
  ...
  void testGameEnd() {
    handler.handleMessage(
        "{\"type\":\"multiplayer_game_start\",\"playerIndex\":1,\"rows\":6,\"cols\":7}");
    assertFalse(handler.isIdle());
