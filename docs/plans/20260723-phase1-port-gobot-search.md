...
- [x] record oracle fixture size/depths, any hidden-state findings, and the exact maintainer
      command to run the clone vs GoBot (`SEARCH=GOBOT EVAL=HANDTUNED` + `eval_java_vs_go.py`)
      — see "Phase-1 Findings" below.

#### Phase-1 Findings

**Oracle fixture** (`src/test/resources/gobot_search_parity.jsonl`, README alongside):
- 412 records, all deterministic `search.ChooseDepth(ctx, state, depth)` results.
- Depths {3: 236, 5: 176}; players {1, 2}; actions MOVE 391 + PLACE_NEUTRALS 21; 179 distinct scores.
- Generated by `../virusgame/backend/arena/cmd/searchparitygen` from diverse-roster self-play
  (budget/tournament/greedy/base/mobility agents), sampled every 3rd ply. `-timeout` gates
  *selection only* — kept records are fully-completed searches → reproducible in Java with no timeout.
- `GoBotSearchParityTest.fixedDepthMoveParityOnFixture`: 412/412 match action AND integer score.

**Hidden-state findings (must be reproduced or moves/scores diverge):**
- `ChooseDepth` does NOT consult the opening book (only `Choose`/`ChooseNodeBudget` do) → every
  record is pure search from the given position.
- Position identity is `board + player + movesLeft + neutralUsed[]`, not the board grid alone.
  `movesLeft` and `neutralUsed[p-1]` feed both `HandTunedEval` (tempo / neutral bonus) and the TT
  key `stateHash` (`add(byte(movesLeft))`, `add(byte(player)|0x20)`) — same lesson as Phase 0's eval.
- Search must run in `long`: GoBot's `int` is 64-bit but the pre-port Java used a 32-bit
  `INF_SCORE = 1<<60` that silently overflowed. Fixed in `GoBotSearcher` (scores are `long`).

**Maintainer command — run the clone vs GoBot (the epic's real floor check):**
```bash
export JAVA_HOME=/usr/lib/jvm/java-21-openjdk-amd64 PATH="$JAVA_HOME/bin:$PATH"
# build the gitignored Go reference binaries from the current virusgame tree:
( cd ../virusgame/backend && go build -o server . && go build -o bot-hoster ./cmd/bot-hoster )
# run the clone (GoBot search + GoBot eval) as challenger over N live games:
SEARCH=GOBOT EVAL=HANDTUNED python3 eval_java_vs_go.py 10
```
`SEARCH=GOBOT EVAL=HANDTUNED` = full GoBot clone by construction. Expect ~even/draws (see
Post-Completion). Harness details / prereqs: `.agents/skills/eval-vs-gobot/SKILL.md`.
