...
# beside every result so the comparison is never left implicit. The in-harness `linear` row of
# --sweep re-measures the same architecture on THIS split and is the apples-to-apples number;
# these are the historical ones the bead was opened against.
...
def group_loss(pred, target, mask, temp, mse_weight):
    ...
    return listnet + mse_weight * mse, listnet.item(), mse.item()
...
def train(data, start, length, tr, va, args, hidden, verbose=True):
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        for chunk in batches(tr, start, length, args.batch_groups, rng):
            ...
            loss, _, _ = group_loss(pred, tgt, mask, temp, args.mse_weight)
            opt.zero_grad()
            loss.backward()
            opt.step()
            tot += loss.item()
...
def fmt(name, m):
    return "%-10s top1 %6.1f%%  rho_mean %6.3f  rho_med %6.3f  MAE %8.1f  R2 %6.3f" % (
        name, 100 * m["top1"], m["spearman_mean"], m["spearman_median"], m["mae"], m["r2"]
    )
...
def main(argv=None):
    ...
    p.add_argument("--mse-weight", type=float, default=0.1,
                   help="weight of the MSE unit anchor; the ranking term is fixed at 1.0")
    ...
    print("loss: ListNet(temp=%g ht units) + %g * MSE   wd=%g dropout=%g"
          % (args.temp, args.mse_weight, args.weight_decay, args.dropout))
    print("linear baseline on record (V3OrderingProbe): top1 %.1f%%  rho %.3f  MAE %.0f  R2 %.3f"
