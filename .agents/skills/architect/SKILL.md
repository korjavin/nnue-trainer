...
- **Execution**: Author a plan file under `docs/plans/` describing the work. Commit the plan, push to `origin`, and run `jules new` to spin up the task. Ensure git remote `origin` is configured. **IMPORTANT**: Always run `jules new` in non-interactive mode by redirecting stdin from `/dev/null` (e.g. `jules new "..." < /dev/null` or `jules new --parallel 2 "..." < /dev/null`) to prevent the CLI from hanging/blocking on terminal stdin inputs.
