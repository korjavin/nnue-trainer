---
name: open-pr-review
description: Comprehensive PR/issue review - analyzes architecture, tests, identifies unrelated changes mixed in, drafts review comment or issue comment. Adapted for Maven/Java projects (nnue-trainer).
...
prompt: |
  ...
  **Your tasks (do all of these):**
  ...
  2. **Run validation & codex review** - do NOT run Java compilation or format checks locally (assume JDK/Java tools do not exist locally):
     - Rely entirely on GitHub Actions CI results for builds, test execution, and Spotless formatting checks.
     - Run the local `codex review --base master` CLI on the worktree (as it is a text-based analyzer and does not require local Java tools).
     - Review the CI log results on GitHub for any check failures.
     Record all CI failures and codex review findings.
  ...
  3. **Architecture analysis** - check for:
     ...
     - Java/Spring patterns and rules
  ...
  **Return a structured report with these sections:**
  ...
  - **Validation results**: test pass/fail, compilation warnings
...
- **Issues** - test failures, compiler/lint errors, bugs (numbered list)
...
**Code examples**: when suggesting fixes, always show proper error handling - never ignore exceptions/errors even in snippets.
...
- **proper error handling in code suggestions** - never ignore exceptions even in example snippets; always show proper error/exception handling
